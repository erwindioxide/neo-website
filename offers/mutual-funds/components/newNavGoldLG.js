...
let newNavGoldLG = document.createElement('div');

// create the sticky btn and modal component
newNavGoldLG.innerHTML = `
  <h6 class=""><a class="nav-link gsBold font-white btn shadow inqSm-Gold" style="background-color:#cc942c; color: #ffffff;" data-toggle="modal" data-target="#inquireformLG">Inquire Now!</a></h6>

  <div class="modal fade" id="inquireformLG" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <p class="text-secondary font-italic claimHeader">
          ...
          <form action="https://rampver.com//mfinquiry-needhelp/#wpcf7-f12057-o1" method="post">
            ...
            <div class="text-center mt-4 mb-4">
              <input type="submit" id="trigger" value="I want to invest!"
                class="text-white btn rfbtn font-weight-bold btn-lg" onclick="submitDetailsLG()" />
              <br />
              <img src="../img/preloader.gif" class="d-none" id="pulse-loadLG"
														style="width: 23%" alt="" />
            </div>
          </form>
        </div>
...
document.getElementById('newNavGoldSetLG').appendChild(newNavGoldLG);

function submitDetailsLG() {
	document.getElementById('pulse-loadLG').classList.remove('d-none');
	setTimeout(function submitDetails() {
	  document.getElementById('pulse-loadLG').classList.add('d-none');
	}, 15000);
  }
...
setTimeout(function load() {
  ...
  document.getElementById('campaignTitleEntrySM').value = title;
  document.getElementById('promotedFundSM').value = title;
  document.getElementById('otherTagsSM').value = title;
}, 3000);